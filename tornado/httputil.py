...
from tornado.util import basestring_type, ObjectDict, PY3, unicode_type
...
class HTTPHeaders(collections.MutableMapping):
    ...
    def __init__(self, *args, **kwargs):
        # Formally, HTTP headers are a mapping from a field name to a "combined field
        # value", which may be constructed from multiple field lines by joining them
        # with commas. In practice, however, some headers (notably Set-Cookie) do not
        # follow this convention, so we maintain a mapping from field name to a list of
        # field lines in self._as_list.
        # self._combined_cache is a cache of the combined field values derived from
        # self._as_list on demand (and cleared whenever the list is modified).
    ...
    def __getitem__(self, name):
        ...
        if header not in self._combined_cache:
            self._combined_cache[header] = ','.join(
                native_str(v) for v in self._as_list[header])
