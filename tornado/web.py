...
class RequestHandler(object):
    ...
    def set_cookie(self, name, value, domain=None, expires=None, path="/",
        ...
        if re.search(r"[\x00-\x20]", value):
            # Legacy check for control characters in cookie values. This check is no longer
            # needed since the cookie library escapes these characters correctly now. It will
            # be removed in the next feature release.
            raise ValueError("Invalid cookie %r: %r" % (name, value))
        for attr_name, attr_value in [("name", name), ("domain", domain), ("path", path)]:
            # Cookie attributes may not contain control characters or semicolons (except when
            # escaped in the value). A check for control characters was added to the
            # http.cookies library in a Feb 2026 security release; as of March it still does
            # not check for semicolons.
            #
            # When a semicolon check is added to the standard library (and the release has had
            # time for adoption), this check may be removed, but be mindful of the fact that
            # this may change the timing of the exception (to the generation of the Set-Cookie
            # header in flush()).
        ...
        for k, v in kwargs.items():
            ...
            elif isinstance(v, unicode_type):
                v_str = v
            else:
                v_str = str(v)
            if re.search(r"[\x00-\x20\x3b\x7f]", v_str):
                raise Cookie.CookieError(
                    "Invalid cookie attribute %s=%r for cookie %r" % (k, v, name))
